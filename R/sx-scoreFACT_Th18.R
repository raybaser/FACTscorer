...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Th18 (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Th18 
...
#' 
#' 
#' @param df A data frame with the FACT-Th18 items, appropriately-named.
...
#'   \item{ThS18}{Thrombo-Cytopenia subscale}
#'   \item{FACT_Th18_TOTAL}{FACT-Th18 Total Score (i.e., PWB+SWB+EWB+FWB+ThS18)}
#'   \item{FACT_Th18_TOI}{FACT-Th18 Trial Outcome Index (e.g., PWB+FWB+ThS18)}
#' }
#' 
#' @references FACT-Th18 Scoring Guidelines, available at \url{http://www.facit.org}
#' 
#' @export
#' 
#' @examples
...
#'                       'ThS18', 'FACT_Th18_TOTAL', 'FACT_Th18_TOI')])
scoreFACT_Th18 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    FACT_Th18_TOTAL_N <- TOTAL_N
    ThS18_N <- AC_N
    ThS18 <- AC
    FACT_Th18_TOTAL <- TOTAL
    FACT_Th18_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ThS18_N, FACT_Th18_TOTAL_N, ThS18, 
            FACT_Th18_TOTAL, FACT_Th18_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ThS18, FACT_Th18_TOTAL, FACT_Th18_TOI))
    }
    return(dfOut)
} 
