...
#' 
#' @details
#' Given a data frame that includes all of the FACT-Th11 (Version 4) items as 
#' variables, appropriately named, this function generates all of the FACT-Th11 
...
#' 
#' 
#' @param df A data frame with the FACT-Th11 items, appropriately-named.
...
#'   \item{ThS11}{Thrombo-Cytopenia subscale}
#'   \item{FACT_Th11_TOTAL}{FACT-Th11 Total Score (i.e., PWB+SWB+EWB+FWB+ThS11)}
#'   \item{FACT_Th11_TOI}{FACT-Th11 Trial Outcome Index (e.g., PWB+FWB+ThS11)}
#' }
#' 
...
scoreFACT_Th11 <- function(df, updateItems = FALSE, keepNvalid = FALSE) {
    ...
    FACT_Th11_TOTAL_N <- TOTAL_N
    ThS11_N <- AC_N
    ThS11 <- AC
    FACT_Th11_TOTAL <- TOTAL
    FACT_Th11_TOI <- TOI
    if (updateItems) {
    ...
    if (keepNvalid) {
        dfOut <- as.data.frame(cbind(dfG, ThS11_N, FACT_Th11_TOTAL_N, ThS11, 
            FACT_Th11_TOTAL, FACT_Th11_TOI))
    } else {
        dfG[, "PWB_N"] <- NULL
        dfG[, "SWB_N"] <- NULL
        dfG[, "EWB_N"] <- NULL
        dfG[, "FWB_N"] <- NULL
        dfG[, "FACTG_N"] <- NULL
        dfOut <- as.data.frame(cbind(dfG, ThS11, FACT_Th11_TOTAL, FACT_Th11_TOI))
    }
    return(dfOut)
} 
